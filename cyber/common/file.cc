...
bool GetProtoFromFile(const std::string &file_name,
                      google::protobuf::Message *message) {
  /*if the file name is a relative path, we cat it with CYBER_PATH to get absolute path,
    which can be defined in environment variable. In common, CYBER_PATH should be like:
    "xxx(your install path)/apollo/". We strongly recommend you use reletive path to your
    apollo install dir in code and config files*/
  auto file_path = file_name;
  if (file_name[0] != '/'){
    file_path = common::GetAbsolutePath(common::GetEnv("CYBER_PATH"),
                                             file_name);
  }
  ...
  if (std::equal(kBinExt.rbegin(), kBinExt.rend(), file_name.rbegin())) {
    return GetProtoFromBinaryFile(file_path, message) ||
           GetProtoFromASCIIFile(file_path, message);
  }

  return GetProtoFromASCIIFile(file_path, message) ||
         GetProtoFromBinaryFile(file_path, message);
}
