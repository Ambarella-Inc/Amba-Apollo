...
TEST(WriterReaderTest, messaging) {
  ...
  auto read_func=[&](const std::shared_ptr<proto::UnitTest>& msg) {
        std::lock_guard<std::mutex> lck(mtx);
        AINFO<<"____________MSG GET!  :"<<&msg;
        recv_msgs.emplace_back(*msg);
        };
  ADEBUG<<"CallbackFunc---------------"<<&read_func;
  attr.set_node_name("reader_a");
  Reader<proto::UnitTest> reader_a(attr, read_func);
  EXPECT_TRUE(reader_a.Init());
  attr.set_node_name("reader_b");
  Reader<proto::UnitTest> reader_b(attr, read_func);
  EXPECT_TRUE(reader_b.Init());
