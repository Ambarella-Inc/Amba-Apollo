...
bool Timer::InitTimerTask() {
  ...
  //task is started with timer
  task_->started_.store(true);
  ...
  if (!started_.exchange(true)) {
    if (InitTimerTask()) {
      ...
      AINFO << "start timer [" << task_->timer_id_ << "]"
            << " Timer task Adress:" << &task_ << "(" << task_.get() << ")"
            << " Timer ref_count : " << task_.use_count();
      ;
    }
  }
}
...
void Timer::Stop() {
  if (started_.exchange(false) && task_) {
    ...
    auto tmp_task = task_;
    {
      ...
      //as the timer is stoped ,its task also should be stopped
      task_->started_.store(false);
      task_.reset();
    }
