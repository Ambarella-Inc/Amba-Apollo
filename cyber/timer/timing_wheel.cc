...
void TimingWheel::Tick() {
  auto& bucket = work_wheel_[current_work_wheel_index_];
  {
    ...
    while (ite != bucket.task_list().end()) {
      auto task = ite->lock();
      if (task) {
        auto task_inner = task.get();
        ADEBUG << "index: " << current_work_wheel_index_
               << " timer id: " << task->timer_id_
               << " from task: " << task_inner
               << " +++++++++++++++++++++ callback func: " << &(task->callback);
        cyber::Async([this, task_inner, task] {
          if (!task->started_.load()) {
            /*
            if the task(also timer) has been stopped,
            we don't need to create new task 
            */
            AWARN << "!!!!!!! Task has been shut down !!!!!!!!! ";
            return;
          }
          /*
          here we capture a copy of the share_ptr task 
          to make sure the callback is valid util it is called
          */
          auto* callback =
              reinterpret_cast<std::function<void()>*>(&(task->callback));
          if (this->running_ && callback) {
            ADEBUG << "index: " << this->current_work_wheel_index_
                   << " from task: " << task_inner
  ...
  if (work_wheel_index >= WORK_WHEEL_SIZE) {
    ...
    if (assistant_ticks == 1 &&
      ...
      ADEBUG << "add task[" << task->timer_id_
             << "] to work wheel. index :" << real_work_wheel_index;
    } else {
      ...
      ADEBUG << "add task[" << task->timer_id_
             << "] to assistant wheel. index : " << assistant_wheel_index;
    }
  } else {
