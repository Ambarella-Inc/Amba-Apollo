...
bool CanbusComponent::Init() {
 ...
 //judge if the lib path is a relative path to CYBER_PATH
  auto vehicle_library_path = FLAGS_load_vehicle_library;
  if (vehicle_library_path[0] != '/'){
    vehicle_library_path = apollo::cyber::common::GetAbsolutePath(apollo::cyber::common::GetEnv("CYBER_PATH"),
                                                                  vehicle_library_path);
  }
  if (!apollo::cyber::common::PathExists(vehicle_library_path)) {
    AERROR << vehicle_library_path << " No such vehicle library";
    return false;
  }
  AINFO << "Load the vehicle factory library: " << vehicle_library_path;
