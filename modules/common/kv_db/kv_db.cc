...
DEFINE_string(kv_db_path, apollo::cyber::common::GetEnv("HOME") + "/dreamview_kv_db.sqlite",
 ...
 public:
  ...
  SqliteWraper() {
    ...
    if (sqlite3_open(FLAGS_kv_db_path.c_str(), &db_) != 0) {
      AERROR << "Can't open Key-Value database: " << sqlite3_errmsg(db_) << "DB path: " << FLAGS_kv_db_path;
      Release();
      return;
    }
